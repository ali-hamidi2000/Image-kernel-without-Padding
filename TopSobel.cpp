...
void moveKernel(Mat img, float TopSobelKernel[3][3], Mat dst) {
      ...
      for (int m = 0; m < 3; m++) {
        ...
        for (int n = 0; n < 3; n++) {
          ...
          if (ValueOne > img.rows && ValueTwo > img.cols) {
            ...
            SumOfIndex += TopSobelKernel[n][m] * img.at<uchar>(ValueOne, ValueTwo);
          }
        }
      }
...
int main() {
  ...
  moveKernel(img, TopSobelKernel, dst);
  imshow("blur", dst);
